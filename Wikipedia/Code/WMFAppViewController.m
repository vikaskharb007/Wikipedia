...
- (BOOL)processUserActivity:(NSUserActivity *)activity animated:(BOOL)animated completion:(dispatch_block_t)done {
    ...
    switch (type) {
        ...
        case WMFUserActivityTypePlaces: {
            ...
            // Ensure Places is in map mode
            [[self placesViewController] updateViewModeToMap];
            
            NSDictionary *userInfo = activity.userInfo;
            NSNumber *latNumber = userInfo[@"WMFLatitude"];
            NSNumber *lonNumber = userInfo[@"WMFLongitude"];
            NSNumber *zoomNumber = userInfo[@"WMFZoom"];
            
            if ([latNumber isKindOfClass:[NSNumber class]] && [lonNumber isKindOfClass:[NSNumber class]]) {
                // Show coordinates if provided in the user activity
                [[self placesViewController] showLocationWithLatitude:latNumber.doubleValue longitude:lonNumber.doubleValue zoom:(zoomNumber ? zoomNumber.doubleValue : 0)];
            } else {
                // Fallback to article URL if present
