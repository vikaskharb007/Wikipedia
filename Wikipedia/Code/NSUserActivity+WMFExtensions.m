...
+ (instancetype)wmf_placesActivityWithURL:(NSURL *)activityURL {
    ...
    for (NSURLQueryItem *item in components.queryItems) {
        ...
        }  else if ([item.name isEqualToString:@"coordinates"]) {
            // Expected format: "lat,lon"
            NSString *coordString = item.value;
            NSArray<NSString *> *parts = [coordString componentsSeparatedByString:@","];
            if (parts.count == 2) {
                NSString *latStr = [parts[0] stringByTrimmingCharactersInSet:[NSCharacterSet whitespaceAndNewlineCharacterSet]];
                NSString *lonStr = [parts[1] stringByTrimmingCharactersInSet:[NSCharacterSet whitespaceAndNewlineCharacterSet]];
                if (latStr.length > 0) { latitude = @([latStr doubleValue]); }
                if (lonStr.length > 0) { longitude = @([lonStr doubleValue]); }
            }
        }
    }
    ...
    // Populate userInfo with parsed values
    NSMutableDictionary *userInfo = [NSMutableDictionary dictionary];
    if (activity.userInfo) {
        [userInfo addEntriesFromDictionary:activity.userInfo];
    }
    if (latitude) { userInfo[@"WMFLatitude"] = latitude; }
    if (longitude) { userInfo[@"WMFLongitude"] = longitude; }
    if (articleURL) { userInfo[@"WMFArticleURL"] = articleURL; }
